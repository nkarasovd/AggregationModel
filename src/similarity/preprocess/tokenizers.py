...
class IdCoder:
    def __init__(self):
        self.unk_symbol = "_|_"
        self.id2name = {0: self.unk_symbol}
        self.name2id = {self.unk_symbol: 0}
        self.fixed = False
    ...
    def encode(self, word: Any) -> int:
        ...
        return self.name2id.get(word, 0)

    def __getitem__(self, item: Any) -> int:
        return self.encode(item)

    def encodes(self, words: List[Any]) -> List[int]:
...
class SimpleTokenizer(Tokenizer):
    ...
    def encode(self, text: List[str]) -> List[int]:
        return self.coder.encodes(text)

    def split(self, text: List[str]) -> List[str]:
        return self.coder.decodes(self.coder.encodes(text))
