...
from src.similarity.preprocess.entry_coders import remove_equals, Entry2Seq
from src.similarity.preprocess.tokenizers import Padding, Tokenizer
...
class SeqCoder:
    ...
    def fit(self, stack_ids: Iterable[int]) -> 'SeqCoder':
        if self.fitted:
            ...
            return self
        # seqs = [self.char_filter(self.entry_to_seq(self.stack_loader(stack_id))) for stack_id in tqdm(stack_ids)]
        seqs = [self.entry_to_seq(self.stack_loader(stack_id)) for stack_id in tqdm(stack_ids)]
    ...
    def _pre_call(self, stack_id: int):
        ...
        for tr in [self.entry_to_seq, self.vocab_control]:
            if tr is not None:
                res = tr(res)
